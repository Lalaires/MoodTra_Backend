...
class Account(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "account_type IN ('guardian','child','admin')", name="account_type_chk"
        ),
...
class StrategyEmotion(Base):
    ...
    strategy_id: Mapped[str] = mapped_column(
        ForeignKey("strategy.strategy_id", ondelete="CASCADE"), primary_key=True
    )
    emotion_id: Mapped[int] = mapped_column(
        ForeignKey("emotion_label.emotion_id", ondelete="CASCADE"), primary_key=True
    )
...
class Activity(Base):
    ...
    account_id: Mapped[UUIDT] = mapped_column(
        ForeignKey("account.account_id", ondelete="CASCADE"), nullable=False
    )
    strategy_id: Mapped[str] = mapped_column(
        ForeignKey("strategy.strategy_id", ondelete="RESTRICT"), nullable=False
    )
    activity_ts: Mapped[datetime] = mapped_column(
    ...
    message_id: Mapped[UUIDT | None] = mapped_column(
...
class ChatMessage(Base):
    ...
    session_id: Mapped[UUIDT | None] = mapped_column(
...
class ChatSession(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "status IN ('active','archived','closed')", name="chat_session_status_chk"
        ),
    )
...
class MoodLog(Base):
    ...
    linked_emotion_id: Mapped[int | None] = mapped_column(
        ForeignKey("emotion_label.emotion_id")
    )
...
class CrisisAlert(Base):
    ...
    crisis_id: Mapped[int] = mapped_column(
        ForeignKey("crisis.crisis_id", ondelete="CASCADE"), nullable=False
    )
    ...
    crisis_alert_ts: Mapped[datetime] = mapped_column(
        TIMESTAMP(timezone=True), nullable=False
    )
    last_msg_ts: Mapped[datetime] = mapped_column(
...
class GuardianChildLink(Base):
    ...
    guardian_id: Mapped[UUIDT] = mapped_column(
        ForeignKey("account.account_id", ondelete="CASCADE"), nullable=False
    )
    child_id: Mapped[UUIDT] = mapped_column(
        ForeignKey("account.account_id", ondelete="CASCADE"), nullable=False
    )
...
class GuardianInvite(Base):
    ...
    status: Mapped[str] = mapped_column(
        Text, server_default=text("'invited'"), nullable=False
    )
    accepted_at: Mapped[datetime | None] = mapped_column(TIMESTAMP(timezone=True))
    accepted_account_id: Mapped[UUIDT | None] = mapped_column(
