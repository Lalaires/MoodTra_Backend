...
def get_latest_crisis_alert(
    account_id: UUID = Depends(get_account_id), db: Session = Depends(get_db)
):
    """
    ...
    try:
        ...
        logger.info(
            f"Retrieved latest crisis alert {result.crisis_alert_id} for account {account_id}"
        )
        ...
        logger.error(
            f"Error retrieving latest crisis alert for account {account_id}: {str(e)}"
        )
        raise HTTPException(
            status_code=500,
...
def acknowledge_crisis_alert(
    ...
    try:
        ...
        logger.error(
            f"Error acknowledging crisis alert {crisis_alert_id} for account {account_id}: {str(e)}"
        )
        raise HTTPException(
            status_code=500,
            detail="Internal server error while acknowledging crisis alert",
        )
